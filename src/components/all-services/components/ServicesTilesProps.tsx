...
type ServicesTilesProps = {
  data: Category[];
  loading: boolean;
  error: string | null;
};

export default function ServicesTilesProps({ data, loading, error }: ServicesTilesProps) {
  return (
    <>
      {loading && (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mt-10">
          {data.map((category: Category) => (
            <Link
              ...
              className="group relative aspect-[4/3] overflow-hidden rounded-2xl border border-[#e5e7eb] shadow-md hover:shadow-xl hover:border-primary transition-all duration-300"
